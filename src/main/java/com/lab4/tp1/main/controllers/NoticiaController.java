...
import com.lab4.tp1.main.services.NoticiaService;
...
public class NoticiaController {
	...
	@PostMapping("/")
	@Transactional
	public ResponseEntity<?> post(@RequestBody Noticia noticia) {
		try {
			return ResponseEntity.status(HttpStatus.OK).body(service.save(noticia));
	...
	@PutMapping("/{id}")
	@Transactional
	public ResponseEntity<?> put(@PathVariable Long id, @RequestBody Noticia noticia) {
		try {
			return ResponseEntity.status(HttpStatus.OK).body(service.update(id, noticia));
